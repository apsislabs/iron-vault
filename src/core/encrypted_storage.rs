...
fn read_encrypted<'a, P: AsRef<path::Path>>(path: P, buffer: &'a mut Vec<u8>, key: &[u8], algorithm: &'static aead::Algorithm) -> &'a[u8] {
...
fn write_encrypted<P: AsRef<path::Path>>(path: P, buf: &[u8], key: &[u8], algorithm: &'static aead::Algorithm) {
...
fn open_data<'a>(data: &'a mut Vec<u8>, key: &[u8], algorithm: &'static aead::Algorithm) -> &'a [u8] {
    ...
    let plaintext = aead::open_in_place(&opening_key, &nonce, &ad[..], nonce_len, &mut data[..]).expect("Should have opened in place properly.");

    return plaintext;
}

fn seal_data<'a>(data: &'a mut Vec<u8>, key: &[u8], algorithm: &'static aead::Algorithm) -> &'a [u8] {
    ...
    let ciphertext_size = aead::seal_in_place(&sealing_key, &nonce, &ad[..], &mut data[..], tag_len).expect("Should have sealed in place properly");
